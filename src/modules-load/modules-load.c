...
static int run(int argc, char *argv[]) {
        ...
        const char *e = getenv("SYSTEMD_MODULES_LOAD_NUM_CONCURRENT");
        if (e) {
                r = safe_atou(e, &num_threads);
                if (r < 0)
                        log_debug_errno(r, "Invalid value in $SYSTEMD_MODULES_LOAD_NUM_CONCURRENT: %s", e);
        }
